...
// use fetch to retrieve the recipes and pass them to init
...
request.onload = function() {
  if(request.status === 200) {
    ...
    console.log('Network request for recipes.json failed with response ' + request.status + ': ' + request.statusText)
  }
};
request.send();
...
// if no recipe match the search term, display a "No result to display" message
    if(finalGroup.length === 0) {
      const heading = document.createElement('h4');
      heading.textContent = 'No result to display!';
...
// Again, if any errors occur we report them in the console.  
  ...
  request.onload = function() {
      if(request.status === 200) {
        ...
        console.log('Network request for "' + recipes.name + '" image failed with response ' + request.status + ': ' + request.statusText);
      }
    };
...
// Give the <h4> textContent equal to the recipes "name" property, but with the first character replaced with the uppercase version of the first character
  heading.textContent = recipes.name.replace(recipes.name.charAt(0), recipes.name.charAt(0).toUpperCase());

  viewRecipe.innerText=recipes.button;

// Set the src of the <img> element to the ObjectURL, and the alt to the recipes"name" property
    image.src = objectURL;
    image.alt = recipes.name;
    viewRecipe.href= recipes.button;

// append the elements to the DOM as appropriate, to add the product to the UI
    main.appendChild(section);
    section.appendChild(image);
    section.appendChild(heading);
    section.appendChild(viewRecipe);
